...
import { LocalBillData } from "./LocalBillForm";

interface LocalBillPreviewProps {
  data: LocalBillData;
}

export function LocalBillPreview({ data }: LocalBillPreviewProps) {
  ...
  const statusClasses = (status: LocalBillData["status"]) =>
    status === "paid"
  ...
  const currencyLabel = (data.currency || "BDT").toUpperCase();
  const formatMoney = (n: number) => {
    const cur = (data.currency || "BDT").toUpperCase();
    const symbol = cur === "BDT" ? "৳" : cur === "EUR" ? "€" : cur === "USD" ? "$" : "";
      ...
      <div className="mt-1 mb-2">
        {(data.agentName || data.agentMobile) && (
          <div className="text-[11px] md:text-sm text-gray-800 leading-tight">
            {data.agentName && (
              <p>
                <span className="font-semibold">Supplier:</span> {data.agentName}
              </p>
            )}
            {data.agentMobile && (
              <p>
        ...
        <table className="w-full text-xs">
          <thead>
            <tr className="border-b border-gray-300">
              ...
              <th className="text-right py-1.5 font-semibold w-24">QUANTITY</th>
              <th className="text-right py-1.5 font-semibold w-24">RATE</th>
              <th className="text-right py-1.5 font-semibold w-32 whitespace-nowrap">AMOUNT {currencyLabel}</th>
            </tr>
          </thead>
          <tbody>
            {data.items.length > 0 ? (
              data.items.map((item, index) => (
                <tr key={index} className="border-b border-gray-200">
                  ...
                  <td className="text-right py-2">
                    {item.isNote ? "" : formatMoney(item.rate)}
                  </td>
                  <td className="text-right py-2">
                    {item.isNote ? "" : formatMoney(item.amount)}
                  </td>
                </tr>
              ))
            ) : (
