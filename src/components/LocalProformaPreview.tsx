...
import { LocalProformaData } from "./LocalProformaForm";

interface LocalProformaPreviewProps {
  data: LocalProformaData;
}

export function LocalProformaPreview({ data }: LocalProformaPreviewProps) {
  ...
  return (
    <Card className="w-full max-w-[210mm] mx-auto p-8 bg-white text-black print:shadow-none print:border-0 print:m-0 print:p-8 font-sans print:w-[210mm]">
      ...
      <div className="mb-4 text-sm">
        ...
        <div className="ml-0">
          <p className="font-medium">{data.toName || "Client Name"}</p>
          <p className="whitespace-pre-line">{data.toAddress || "Client Address"}</p>
        </div>
      </div>
        ...
        <table className="w-full border-2 border-black text-sm">
          <thead>
            <tr className="border-b-2 border-black">
              ...
              <th className="border-r-2 border-black p-2 text-left font-semibold">Item description</th>
              <th className="border-r-2 border-black p-2 text-left font-semibold w-32">Quantity in Kg</th>
              <th className="border-r-2 border-black p-2 text-left font-semibold w-32">Unit Price TK/Kg</th>
              <th className="p-2 text-left font-semibold w-32">Total Price TK</th>
            </tr>
          </thead>
          <tbody>
            {data.items.length > 0 ? (
              data.items.map((item, index) => (
                <tr key={index} className="border-b border-black">
                  ...
                  <td className="border-r-2 border-black p-2 whitespace-pre-line">{item.itemDescription}</td>
                  <td className="border-r-2 border-black p-2 text-right">{item.quantityInKg.toLocaleString()}</td>
                  <td className="border-r-2 border-black p-2 text-right">{item.unitPriceTk.toFixed(2)}</td>
                  <td className="p-2 text-right">{item.totalPriceTk.toFixed(2)}</td>
                </tr>
              ))
            ) : (
      ...
      {data.paymentTerms && (
        <div className="mb-8 text-sm">
          <p><span className="font-semibold">Payment terms:</span> {data.paymentTerms}</p>
        </div>
      )}
